import { AElementComponent, FlowContent, IElementComponent, toKebabCase } from "@vanilla-ts/core";
...
/**
 * Type of the content to be used for the title of the dialog box. No further assumptions are made
 * regarding the design and functionality. If used, it should comply with the conventions for dialog
 * boxes of the respective operating system on which the application is running.\
 * If `StdDlgTitle` is a function, it will be called with the `StdDialog` instance into which the
 * title content is to be mounted, so it is possible to use the dialog instance for advanced
 * customization of the title (parts). This can be done, for example, by installing a `once` event
 * handler for the `dlg-shown` event on `StdDialog.Dialog` since it is guaranteed that the dialog
 * will have its final form when this event is triggered.\
 ...
 */
export type StdDlgTitle = string | FlowContent | (string | FlowContent)[] | ((dlg: StdDialog) => (string | FlowContent)[]);
 ...
 * `Text` component or an instance of `INodeComponent<HTMLElement>`.
 */
export type StdDlgContent = string | FlowContent | (string | FlowContent)[] | ((dlg: StdDialog) => (string | FlowContent)[]);
...
export type StdDlgOptions = {
    /**
     * Component(s)/text(s) for the title bar of the dialog box, either as a single value, as an
     * array of values or as a function. No further assumptions are made regarding the design and
     * functionality. If used, it should comply with the conventions for dialog boxes of the
     * respective operating system on which the application is running.
     */
    Title?: StdDlgTitle;
    /**
